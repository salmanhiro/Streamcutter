...
def main():
    ...
    gc_central_feh, _ = extract_gc_region.select_gc_region_feh(
        gc_df, FM_T_sel, RV_T_sel, radius_deg=None, use_tidal_radius=True, plot=True
    )
    print(f"Median [Fe/H] of {gc_name}: median={np.median(gc_central_feh):.2f}, std={np.std(gc_central_feh):.2f}, min={np.min(gc_central_feh):.2f}, max={np.max(gc_central_feh):.2f}")
    # TODO: Address double peak
    ...
    feh_mean = np.median(gc_central_feh)
    ...
    with open(f"{out_dir}/{gc_name}_selection_notes.txt", "w") as f:
        ...
        f.write(f"GC region metallicity [Fe/H]: median={np.median(gc_central_feh):.2f}, std={np.std(gc_central_feh):.2f}, min={np.min(gc_central_feh):.2f}, max={np.max(gc_central_feh):.2f}\n")
